...
export const DAMAGE_REGEX =
  /^(?<accum>\+)?(?<roll>\d+)(?<D>d\d*)?(?<adds>[\–\-+]@?\w+)?(?<mult>[×x\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*?)(?<costs>\*(costs|per)? \d+ ?[\w\(\) ]+)?(?<follow>,.*)?$/i
...
export const DERIVED_DAMAGE_REGEX =
  /^(?<accum>\+)?(?<att>sw|thr)()(?<adds>[\–\-+]@?\w+)?(?<mult>[×x\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*?)(?<costs>\*(costs|per)? \d+ ?[\w\(\) ]+])?(?<follow>,.*)?$/i
...
export function parseForRollOrDamage(str, overridetxt) {
  ...
  if (!!a) {
    ...
    let dmap = DamageTables.translate(actualType.toLowerCase())
    if (!dmap) {
      dmap = DamageTables.translate(extDamageType?.toLowerCase())
      if (!!dmap) {
